document.addEventListener('DOMContentLoaded', () => {
    ...
    navButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            // If we are showing the graph page, fetch data and draw the chart
            if (targetPage === 'graph') {
    ...
    riskForm.addEventListener('submit', async (e) => {
        ...
        // 4. Send data to the backend server to save in MongoDB
        try {
            ...
            if (response.ok) {
                ...
                latestAssessment = await response.json(); // Store the saved data
            } else {
    ...
    async function fetchLatestAssessmentAndDrawChart() {
        // If we don't have the latest data, fetch it from the server
        if (!latestAssessment) {
            try {
                const response = await fetch('http://localhost:3000/api/assessments');
                const allData = await response.json();
                if (allData.length > 0) {
                    latestAssessment = allData[0]; // The first one is the most recent
                } else {
        ...
        // Now draw the chart with the latest data
    ...
    function drawSolutionChart(data) {
        ...
        // Create different solution plans based on risk level
        if (data.riskLevel === "Low") {
            chartData = [150, 60, 49, 30, 14]; // Cardio, Strength, Sleep, Meditation, Healthy Meals
            solutionText = `
                <li><strong>Cardio:</strong> Aim for 150 minutes of moderate activity per week.</li>
                <li><strong>Strength Training:</strong> Incorporate 2 sessions (60 min total) per week.</li>
                <li><strong>Sleep:</strong> Ensure 7 hours (49 total) per night.</li>
                <li><strong>Meditation:</strong> Practice mindfulness for 30 minutes weekly.</li>
                <li><strong>Healthy Meals:</strong> Focus on 14 balanced, healthy meals.</li>`;
        } else if (data.riskLevel === "Medium") {
            chartData = [180, 90, 52, 60, 18];
            solutionText = `
                <li><strong>Cardio:</strong> Increase to 180 minutes of activity per week.</li>
                <li><strong>Strength Training:</strong> Aim for 3 sessions (90 min total) per week.</li>
                <li><strong>Sleep:</strong> Prioritize 7.5 hours (52 total) per night.</li>
                <li><strong>Meditation:</strong> Practice mindfulness for 60 minutes weekly to manage stress.</li>
                <li><strong>Healthy Meals:</strong> Focus on 18 balanced, low-sodium meals.</li>`;
        } else { // High Risk
            chartData = [200, 120, 56, 90, 21];
            solutionText = `
                <li><strong>Cardio:</strong> Aim for 200+ minutes of gentle activity per week (consult a doctor).</li>
                <li><strong>Strength Training:</strong> Focus on 3-4 light sessions (120 min total).</li>
                <li><strong>Sleep:</strong> Target 8 hours (56 total) per night for recovery.</li>
                <li><strong>Meditation:</strong> Critical for stress reduction, aim for 90 minutes weekly.</li>
                <li><strong>Healthy Meals:</strong> Strictly follow a heart-healthy diet for all 21 main meals.</li>`;
        }

        solutionList.innerHTML = solutionText;

        if (solutionChart) {
            solutionChart.destroy(); // Destroy old chart before creating a new one
        }
